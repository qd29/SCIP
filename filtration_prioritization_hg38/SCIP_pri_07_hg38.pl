  ...
  if ($coding_in==1){
   ...
   if ($#split7>=0){
    for (my $i=0; $i<=$#split7; $i++){
     ...
     my $trans_out="<tr>";
     if ($split11[$i]==1){
      $trans_out="$trans_out<td><a href='https://ensembl.org/Homo_sapiens/Transcript/Summary?db=core;g=$split1[0];t=$split7[$i]' target='_blank'>$split7[$i]</a> <span class='label label-warning'>MANE</span></td>";
     }
     else{
      $trans_out="$trans_out<td><a href='https://ensembl.org/Homo_sapiens/Transcript/Summary?db=core;g=$split1[0];t=$split7[$i]' target='_blank'>$split7[$i]</a></td>";
     }
   ...
   if ($dom==1){
    print out1 "<a href='https://ensembl.org/Homo_sapiens/Gene/Summary?db=core;g=$split1[0]' target='_blank'>$split1[1]</a><br/><span class='label label-warning'>Dominant</span>";
   }
   else{
    print out1 "<a href='https://ensembl.org/Homo_sapiens/Gene/Summary?db=core;g=$split1[0]' target='_blank'>$split1[1]</a>";
   }
