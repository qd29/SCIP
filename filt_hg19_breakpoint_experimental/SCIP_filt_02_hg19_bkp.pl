...
open out1, ">./user_data/$name.bkp.filt_step02.txt";
foreach my $chr (@chr){
 print "SCIP Filtration Module script 02 processing hg19 (breakpoint-based) chr$chr\n";
 my @exon;
 ...
 open file1, "<./user_data/$name.bkp.filt_step01.txt";
 while (<file1>){
  chomp;
  my @split1=split /\t/,$_;
  if ($split1[0] eq $chr){
